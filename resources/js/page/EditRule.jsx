...
import Status from "../component/AddRule/Status";
...
import { getRequest, postRequest, updatePostRequestUsingId } from "../api";
...
const EditRule = () => {
    ...
    const metaData = JSON.parse(discount?.metafield?.value)?.ruleData
    const { id: routeId } = useParams();
    const [discountStatus, setDiscountStatus] = useState(discount?.discount?.status);
    const navigate = useNavigate();
    const errorRef = useRef(null);
    const navigation = useNavigation();
    const [loading, setLoading] = useState(false);
    const [active, setActive] = useState({});
    const [numberOfTime, setNumberOfTime] = useState(metaData?.discountUses?.numberOfTime ? true : false);
    const [endDateSelected, setEndDateSelected] = useState(metaData?.activeDate?.endDate ? true : false);
    const [showShippingMethods, setShowShippingMethods] = useState(metaData?.shippingMethods?.label);
    ...
    const submitForm = async () => {
        const id = discount?.id.split('/').pop();
        ...
        try {
            if (discount?.discount?.__typename === "DiscountCodeApp") {
                const data = await postRequest(`/api/post-rule/code/${id}`, formData);
                const postResponseCode = data?.discount?.data?.discountCodeAppUpdate
                ...
                const data = await postRequest(`/api/post-rule/automatic/${id}`, formData);
                const postResponseAutomatic = data?.discount?.data?.discountAutomaticAppUpdate;
    ...
    useEffect(() => {
        ...
        const formatter = new Intl.DateTimeFormat('en-US', {
            timeZone: metaData?.shopTimeZone,
            timeZoneName: "short"
        });
        const estTimezone = formatter.formatToParts(new Date()).find(part => part.type === "timeZoneName")?.value;
        setFormData({
            routeId: metaData?.routeId,
            metaFieldId: discount?.metafield?.id,
            discountName: metaData?.discountName,
            discountType: metaData?.discountType,
            discountCode: metaData?.discountCode,
            shopTimeZone: metaData?.shopTimeZone,
            timeZoneFormat: estTimezone,
            shippingMethods: {
                ...metaData?.shippingMethods
            },
            discountUses: {
                numberOfTime: metaData?.discountUses?.numberOfTime,
                oneUse: metaData?.discountUses?.oneUse
            },
            combinations: {
                productDiscounts: metaData?.combinations?.productDiscounts,
                orderDiscounts: metaData?.combinations?.orderDiscounts,
            },
            activeDate: {
                ...metaData?.activeDate
            },
    ...
    const handleSaveBarClose = () => {
        shopify.saveBar.hide('my-save-bar');
    };

    const discountCodeActivate = async (discount) => {
        const toastId = toast.loading("Saving...");
        const id = discount?.gid.split('/').pop();
        try {
            setLoading(true);
            if (discount?.type === "DiscountCodeApp") {
                const data = await updatePostRequestUsingId(`/api/update-rule/code/status/activate/${id}`);
                if (data?.activated?.data?.discountCodeActivate?.codeDiscountNode?.id) {
                    ...
                    setDiscountStatus(data?.activated?.data?.discountCodeActivate?.codeDiscountNode?.codeDiscount?.status)
                }

            } else {
                const data = await updatePostRequestUsingId(`/api/update-rule/automatic/status/activate/${id}`);
                if (data?.activated?.data?.discountAutomaticActivate?.automaticDiscountNode?.id) {
                    ...
                    setDiscountStatus((data?.activated?.data?.discountAutomaticActivate?.automaticDiscountNode?.automaticDiscount?.status))
                }
            }
        } catch (error) {
            console.error(error);
    ...
    const discountCodeDeactivate = async (discount) => {
        const id = discount?.gid.split('/').pop();
        const toastId = toast.loading("Saving...");
        try {
            setLoading(true);
            if (discount?.type === "DiscountCodeApp") {
                const data = await updatePostRequestUsingId(`/api/update-rule/code/status/deactivate/${id}`);
                if (data?.activated?.data?.discountCodeDeactivate?.codeDiscountNode?.id) {
                    toast.success("Saved", {
                        style: {
                            color: 'green',
                        },
                    ...
                    setDiscountStatus(data?.activated?.data?.discountCodeDeactivate?.codeDiscountNode?.codeDiscount?.status)
                }
            } else {
                const data = await updatePostRequestUsingId(`/api/update-rule/automatic/status/deactivate/${id}`);
                if (data?.activated?.data?.discountAutomaticDeactivate?.automaticDiscountNode?.id) {
                    toast.success("Saved", {
                        style: {
                            color: 'green',
                        },
                    ...
                    setDiscountStatus(data?.activated?.data?.discountAutomaticDeactivate?.automaticDiscountNode?.automaticDiscount?.status)
                }
            }
            ...
            <Page
                ...
                primaryAction={
                    discountStatus == "ACTIVE" ?
                        <Button tone='critical' size='micro' disabled={loading}
                            onClick={() => discountCodeDeactivate({ gid: discount?.id, type: discount?.discount?.__typename })}
                        >
                            Deactivate
                        </Button>
                        : discountStatus == "SCHEDULED" ?
                            ...
                            :
                            <Button tone='success' size='micro' disabled={loading}
                                onClick={() => discountCodeActivate({ gid: discount?.id, type: discount?.discount?.__typename })}
                            >
                                Activate
                            </Button>
                                            ...
                                            }
                                            {
                                                conditions?.specificDates?.map((condition, index) => (
                                                    <Box key={index}>
                                                        ...
                                                        <SpecificDatesCondition conditionIndex={conditionIndex} condition={condition} conditionKey={'specificDates'} index={index} setFormData={setFormData} options={selectOptionsString} shopTimeZone={formData?.shopTimeZone}
                                                        />
                                                    </Box>
                                                ))
                                            }
                                            ...
                                            }
                                            {
                                                conditions?.hours?.map((condition, index) => (
                                                    <Box key={index}>
                                                        ...
                                                        <HoursCondition conditionIndex={conditionIndex} condition={condition} conditionKey={'hours'} index={index} setFormData={setFormData} options={selectOptionsLocalTime}
                                                        />
                                                    </Box>
                                            ...
                                            }
                                            {
                                                conditions?.weekday?.map((condition, index) => (
                                                    <Box key={index}>
                                                        ...
                                                        <WeekdayCondition conditionIndex={conditionIndex} condition={condition} conditionKey={'weekday'} index={index} setFormData={setFormData} options={selectOptionsLocalTime}
                                                        />
                                                    </Box>
                            ...
                            <Card roundedAbove="sm">
                                ...
                                <Box paddingBlockStart="500" paddingBlockEnd={'500'}>
                                    <ButtonGroup segmented>
                                        <Button
                                            disabled={discount?.discount?.__typename === 'DiscountAutomaticApp'}
                                        >
                                            Discount code
                                        </Button>
                                        <Button
                                            disabled={discount?.discount?.__typename === 'DiscountCodeApp'}
                                        >
                                            Automatic discount
                                        </Button>
                                    </ButtonGroup>
                                    ...
                                    <BlockStack gap="200">
                                        <Grid gap={500}>
                                            <Grid.Cell columnSpan={{ xs: 6, sm: 6, md: 6, lg: 6, xl: 6 }}>
                                                <DateSelect setFormData={setFormData} updateField={'startDate'} label={'Start date'} formDate={formData?.activeDate?.startDate} />
                                            </Grid.Cell>
                                            <Grid.Cell columnSpan={{ xs: 6, sm: 6, md: 6, lg: 6, xl: 6 }}>
                                                <TimeSelect setFormData={setFormData} updateField={'startTime'} formTime={formData?.activeDate?.startTime} label={'Start time ' + formData?.timeZoneFormat} />
                                            </Grid.Cell>
                                        </Grid>
                                    </BlockStack>
                                </Box>
...

                                {
                                    ...
                                    <Box paddingBlockStart="500">
                                        <BlockStack gap="200">
                                            <Grid gap={500}>
                                                <Grid.Cell columnSpan={{ xs: 6, sm: 6, md: 6, lg: 6, xl: 6 }}>
                                                    <DateSelect setFormData={setFormData} updateField={'endDate'} label={'End date'} formDate={formData?.activeDate?.endDate} />
                                                </Grid.Cell>
                                                <Grid.Cell columnSpan={{ xs: 6, sm: 6, md: 6, lg: 6, xl: 6 }}>
                                                    <TimeSelect setFormData={setFormData} updateField={'endTime'} label={'End time ' + formData?.timeZoneFormat} formTime={formData?.activeDate?.endTime} />
                                                </Grid.Cell>
                                            </Grid>
                                        </BlockStack>
                                    </Box>
